        ...
        <!-- Sidebar Navigation -->
         <div class="md:col-span-3 space-y-2">
            <button *ngFor="let tab of tabs" 
                    ...
                    [class]="activeTab === tab.id ? 'bg-white text-primary shadow-sm border border-slate-200' : 'text-text-muted hover:text-text hover:bg-slate-100/50'"
                    class="w-full flex items-center space-x-3 px-4 py-3 rounded-ui transition-all font-bold text-sm">
           ...
           <!-- Account Settings -->
            <div *ngIf="activeTab === 'account'" class="card-stitch p-8 bg-white space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
               <div class="flex items-center justify-between">
                  ...
                  <button class="text-primary font-bold text-xs uppercase tracking-widest hover:underline">Update Profile</button>
               </div>

               <div class="flex flex-col md:flex-row items-start md:items-center space-y-6 md:space-y-0 md:space-x-8">
                  <div class="relative group cursor-pointer">
                     <div class="w-24 h-24 rounded-3xl bg-slate-100 border-2 border-dashed border-slate-200 flex items-center justify-center overflow-hidden transition-all group-hover:border-primary/50">
                  ...
                  <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
                     <div class="space-y-1.5">
                        <label class="text-[10px] font-black text-text-muted uppercase tracking-widest ml-1">Full Name</label>
                        <input type="text" value="Alex Rivera" class="input-stitch font-bold text-slate-700">
                     </div>
                     <div class="space-y-1.5">
                        <label class="text-[10px] font-black text-text-muted uppercase tracking-widest ml-1">Role Type</label>
                        <input type="text" value="Administrator" readonly class="input-stitch font-bold text-slate-400 bg-slate-100 cursor-not-allowed">
                     </div>
                     <div class="space-y-1.5 md:col-span-2">
                        <label class="text-[10px] font-black text-text-muted uppercase tracking-widest ml-1">Email Address</label>
                        <input type="email" value="alex.rivera@example.com" class="input-stitch font-bold text-slate-700">
                     </div>
                  </div>
               </div>
            </div>
           ...
           <!-- Notification Preferences -->
               ...
               <div class="space-y-4">
                  <div *ngFor="let pref of notifications" class="flex items-center justify-between p-4 rounded-xl bg-slate-50/50 border border-slate-100 transition-all hover:bg-slate-50">
                     <div class="flex items-center space-x-4">
                        <div class="w-10 h-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-text-muted shadow-sm">
                        ...
                        <div>
                           ...
                           <p class="text-[11px] text-text-muted font-medium mt-0.5">{{ pref.desc }}</p>
                        </div>
                     </div>
                     <button class="w-12 h-6 rounded-full bg-primary relative transition-all flex items-center">
            ...
            <div *ngIf="activeTab === 'sync'" class="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
               <div class="card-stitch p-8 bg-white space-y-6 relative overflow-hidden">
                  <div class="absolute top-0 right-0 w-32 h-32 bg-primary/5 rounded-bl-full -mr-16 -mt-16 border border-primary/20"></div>
                  
                  <div class="flex items-center space-x-4">
                     <div class="w-14 h-14 rounded-2xl bg-success/10 flex items-center justify-center shadow-lg shadow-success/20 border border-success/20">
                     ...
                     <div>
                        ...
                        <div class="flex items-center space-x-2 mt-0.5">
                           ...
                           <span class="text-[11px] font-bold text-success-text uppercase tracking-widest">Connected & Healthy</span>
                        </div>
                     </div>
                  </div>
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                     <div class="p-4 rounded-xl border border-slate-100 bg-slate-50/30">
                        <span class="text-[10px] font-black text-text-muted uppercase tracking-widest block mb-2">Target Spreadsheet</span>
                        <p class="text-sm font-bold text-slate-700 truncate">Ecommerce Admin DB - Main V2</p>
                     </div>
                     <div class="p-4 rounded-xl border border-slate-100 bg-slate-50/30">
                        <span class="text-[10px] font-black text-text-muted uppercase tracking-widest block mb-2">Last Synchronized</span>
                        <p class="text-sm font-bold text-slate-700">Mar 10, 2026 - 08:45 AM</p>
                     </div>
                  </div>

                  <div class="pt-6 border-t border-slate-100 flex justify-between items-center">
                     <button class="text-primary font-bold text-xs uppercase tracking-widest hover:underline px-2 py-1">Advanced Connection Settings</button>
                     <button class="bg-primary hover:bg-blue-700 text-white px-6 py-2.5 rounded-ui shadow-md hover:shadow-lg transition-all font-bold text-sm flex items-center space-x-2 active:scale-95">
               ...
               <div class="card-stitch bg-slate-900 text-white p-6 font-mono text-[11px] space-y-2 opacity-90 shadow-2xl">
                  <p class="text-success">[08:45:01] INFO: Connecting to Google Sheets API...</p>
                  <p class="text-success">[08:45:02] INFO: Fetching range 'Orders!A1:Z500'</p>
                  <p class="text-success">[08:45:03] SUCCESS: Successfully synchronized 432 entries.</p>
                  <p class="text-text-muted">[08:45:03] IDLE: Awaiting next trigger...</p>
               </div>
            </div>
